...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-gray-100 flex items-center justify-center min-h-screen m-0 p-0">
        ...
        <h1 class="text-4xl font-bold text-center mb-8 border-4 border-double border-indigo-500 rounded-md">Bangor University</h1>
                ...
                <div class="max-w-2xl bg-white shadow-lg rounded-lg overflow-hidden flex flex-col">
                    ...
                    <div class="p-6 flex flex-col flex-grow">
                        ...
                        <div class="mt-4 space-y-2 flex-grow">
                            <p class="text-gray-700"><strong>Entry Requirement:</strong> {{ $post->CEntryRequirement }}</p>
                            <p class="text-gray-700"><strong>Fees & Funding:</strong> {{ $post->Linkhref }}</p>
                            <p class="text-gray-700"><strong>Course Duration:</strong> {{ $post->CDuration }}</p>
                            <p class="text-gray-700"><strong>Intakes:</strong> {{ $post->CIntake }}</p>
                        </div>
                        
                        <button class="mt-4 w-full bg-blue-600 text-white py-2 rounded hover:bg-blue-700 mb-4" onclick="location.href='{{ $post->SubLinkhref }}'">Enroll Now</button>
                    </div>
                </div>
                @endforeach
            </div>
